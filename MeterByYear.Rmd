---
title: "Voltage - Individual Meter Report"
output: html_document
params:
  meteridentifier: "200420"
  cut1: 0
  cut2: 114
  cut3: 120
  cut4: 126
...
sSQLcmd <- paste("
                    SELECT 
                       ROW_NUMBER() OVER (ORDER BY ReadDate) AS RowID
                      ,[ODMReporting].[ODM].[IntervalReads].[MeterIdentifier] as 'Meter ID'
                      ,[ODMReporting].[ODM].[IntervalReads].[ReadDate] as 'Read Date/Time'
                      ,[ODMReporting].[ODM].[IntervalReads].[ReadValue] as 'Voltage'
                      ,[ODMReporting].[ODM].[IntervalReads].[LocationNumber] as 'Location ID'
	                    ,[MDM].[dbo].[Substation].[SubstationName] as 'Substation'
                    FROM [ODMReporting].[ODM].[IntervalReads]
                    join [MDM].[dbo].[Location]
                    on [ODMReporting].[ODM].[IntervalReads].LocationNumber = [MDM].[dbo].[Location].LocationNumber
                    join [MDM].[dbo].[Substation]
                    on [MDM].[dbo].[Location].SubstationId = [MDM].[dbo].[Substation].SubstationId
                    where MeterIdentifier = '", params$meteridentifier ,"' 
                    and (UOM = 'Voltage')
                    and (ReadQualityCode = 3)
                    and (ReadDate > '",dtReadDayFrom,"' AND ReadDate <= '",dtReadDayTo,"')
                    order by MeterIdentifier asc, ReadDate asc
                  "
                   ,sep="" ) 
```
...
volts_raw$IsWeekEnd  <- wday(volts_raw$i.dtReadDay, label = TRUE)

volts_raw$IsWeekEnd <- with(volts_raw, ifelse(wday == "Sat", 1,
                                          ifelse(wday == "Sun", 1 , 0)))
...
### Summary Statistics for Meter - `r params$meteridentifier` | Year - `r year(params$startdate)`
...
```{r Load Shape, fig.height=96, fig.width=16}
      ggplot(data=volts_raw,aes(x=hm.end ,y=volts_raw$V, colour=volts_raw$IsWeekEnd)) +
        geom_point(alpha = 1/1) +
        #scale_y_continuous(limits = c(110,130)) +
        facet_wrap(i.month~i.day, ncol=4) +
        theme_bw() +
        ylab("Voltage Scaled") +
        xlab("Day") +
